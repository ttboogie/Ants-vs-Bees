...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        for i in range(self.min_range):
            ...
            if holder == beehive:
                return None
        while holder != beehive and distance>=0:
            if holder.bees:
                return rANTdom_else_none(holder.bees)
            holder=holder.entrance
            distance-=1
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.imposter:
            ...
            holder=self.place.exit
            while holder!=None:
                if holder.ant!=None:
                    if holder.ant.doubled==False:
                        holder.ant.damage*=2
                        if isinstance(holder.ant, ContainerAnt) and holder.ant.contained_ant!=None:
                            if holder.ant.contained_ant.doubled==False:
                                holder.ant.contained_ant.damage*=2
                                holder.ant.contained_ant.doubled=True
                        holder.ant.doubled=True
                    elif isinstance(holder.ant, ContainerAnt) and holder.ant.contained_ant!=None:
                        if holder.ant.contained_ant.doubled == False:
                            holder.ant.contained_ant.damage *= 2
                            holder.ant.contained_ant.doubled = True
                holder=holder.exit
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if not self.direction: #checks if the direction is false (false = backwards)
            destination = self.place.entrance #go backwards
        # END EC
        if self.blocked():
...
def make_slow(action, bee):
    ...
    def actions(gamestate):
        if gamestate.time%2==0: #every other turn
            action(gamestate)
    return actions
    # END Problem EC
...
def make_scare(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def actions(gamestate):
        bee.scared=True #make it so that bee can't be scared again
        bee.direction=False #change direction
        action(gamestate)
    return actions
    # END Problem EC
